const IssuesAPI = {
  ...
  getIssuesAPIForSites(cfg) {
    const el = $(cfg.el)[0];
    $(el).append(
      '<div class="loading"><i class="fa fa-cog fa-2x fa-spin"></i><p>正在加载</p></div>',
    );
    this.requestIssuesAPI(
      cfg.api,
      function (data) {
            ...
            for (j = 0; j < issues.length; j++) {
              ...
              if (issue.screenshot && issue.screenshot.length > 0) {
                imgTag =
                  ...
                  issue.screenshot +
                  '" onerror="javascript:this.src=\'https://image.thum.io/get/width/1024/crop/768/' +
                  issue.url +
                  "';\"/></div>";
              } else {
              ...
              infoTag +=
                ...
                "</span></div>";
              const cardTag =
                "<a class='site-card' target='_blank' href='" +
                issue.url +
                "'>" +
                imgTag +
                infoTag +
    ...
    this.requestIssuesAPI(
      ...
      function (data) {
        ...
        if (data.length > 0) {
          for (i = 0; i < data.length; i++) {
            const a =
              '&nbsp;&nbsp;<a class="comments" target="_blank" href="' +
              data[i].html_url +
              '"><i class="fa fa-comment-dots fa-fw"></i>' +
              data[i].comments +
              "</a>";
            const meta =
              '<div class="meta"><p></p><p>' +
              data[i].title +
              a +
              "</p><p></p></div>";
